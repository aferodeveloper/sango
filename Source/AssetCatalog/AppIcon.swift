...
class AppIcon: NSObject {
    ...
    func generateImagesForPlatforms(_ platforms: [String], fromImage image: NSImage) throws {
        ...
        for platform in platforms {
            ...
            let jsonData = try ContentsJSON(forType: IconAssetType.appIcon, andPlatforms: [platform])
...
//                                                     isDirectory: true)
            let saveUrl = url.appendingPathComponent("\(platform)/\(name).appiconset",
                isDirectory: true)
    ...
    ///   - url: The URL to save the catalog to
    /// - Throws: An AppIconError
    func saveCombinedAssetCatalog(named name: String, toUrl url: URL) throws -> URL {
//        let saveUrl = url.appendingPathComponent("\(appIconDir)/Combined/\(name).appiconset", isDirectory: true)
        ...
        for (_, images) in images {
            ...
            var contentsJson = try ContentsJSON(forType: IconAssetType.appIcon,
